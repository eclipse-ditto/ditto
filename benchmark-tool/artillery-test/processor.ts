...
import { closeAllWebSocketConnections } from './channels/websocket-channel';
...
function setupAbortWatcher() {
    ...
    abortWatcher = fs.watch(ABORT_FILE_DIR, (eventType, filename) => {
        if (filename === ABORT_FILE_NAME && fs.existsSync(ABORT_FILE)) {
...
export async function beforeTest(context: any, events: any): Promise<void> {
    ...
    if (config.cleanupOnly) {
        console.log('[Setup] Cleanup-only mode, no scenarios to run.');
        try {
