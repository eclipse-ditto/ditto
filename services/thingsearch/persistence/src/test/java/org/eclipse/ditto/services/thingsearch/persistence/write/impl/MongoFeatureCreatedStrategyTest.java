...
public final class MongoFeatureCreatedStrategyTest extends AbstractMongoEventToPersistenceStrategyTest {

    private final MongoFeatureCreatedStrategy strategy = new MongoFeatureCreatedStrategy();
    ...
    public void policyUpdates() {
        ...
        verifyPermissionCallForSchemaVersion(() -> any(ResourceKey.class), ArgumentMatchers::anyString,
                1 + TestConstants.ThingEvent.FEATURE_CREATED.getFeature().getProperties().get().getSize());
        ;
    }

    private ProcessableThingEvent<FeatureCreated> thingEvent() {
        return ProcessableThingEvent.newInstance(TestConstants.ThingEvent.FEATURE_CREATED, version);
    }
