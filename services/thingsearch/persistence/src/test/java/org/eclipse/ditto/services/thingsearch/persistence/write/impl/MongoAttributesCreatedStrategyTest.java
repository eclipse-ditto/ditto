...
public final class MongoAttributesCreatedStrategyTest extends AbstractMongoEventToPersistenceStrategyTest {

    private final MongoAttributesCreatedStrategy strategy = new MongoAttributesCreatedStrategy();
    ...
    public void thingUpdates() throws Exception {
        ...
        verify(indexLengthRestrictionEnforcer).enforceRestrictions(thingEvent().getThingEvent().getCreatedAttributes());
    }
    ...
    public void policyUpdates() {
        ...
        verifyPermissionCallForSchemaVersion(() -> any(ResourceKey.class), ArgumentMatchers::anyString,
                1 + TestConstants.ThingEvent.ATTRIBUTES_CREATED.getCreatedAttributes().getSize());
    }

    private ProcessableThingEvent<AttributesCreated> thingEvent() {
        return ProcessableThingEvent.newInstance(TestConstants.ThingEvent.ATTRIBUTES_CREATED, version);
    }
