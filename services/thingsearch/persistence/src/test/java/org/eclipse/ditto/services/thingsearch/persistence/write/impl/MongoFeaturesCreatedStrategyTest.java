...
public final class MongoFeaturesCreatedStrategyTest extends AbstractMongoEventToPersistenceStrategyTest {

    private final MongoFeaturesCreatedStrategy strategy = new MongoFeaturesCreatedStrategy();
    ...
    public void policyUpdates() {
        ...
        verifyPermissionCallForSchemaVersion(() -> any(ResourceKey.class),
                ArgumentMatchers::anyString,
                expectedPolicyUpdates());
    }

    private int expectedPolicyUpdates() {
        return TestConstants.ThingEvent.FEATURES_CREATED.getFeatures().getSize()
                + TestConstants.ThingEvent.FEATURES_CREATED.getFeatures().stream().map(ft ->
                ft.getProperties().get().getSize()).reduce(0, (a, b) -> a + b);
    }

    private ProcessableThingEvent<FeaturesCreated> thingEvent() {
        return ProcessableThingEvent.newInstance(TestConstants.ThingEvent.FEATURES_CREATED, version);
    }
