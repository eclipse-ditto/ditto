...
public class FeaturesDocumentBuilderTest {
    ...
    public void addFeatures() {
        ...
        Assertions.assertThat(((List<Document>) doc.get(FIELD_INTERNAL)).get(0).get("k"))
                .isEqualTo(FIELD_FEATURE_PROPERTIES_PREFIX_WITH_ENDING_SLASH + PROP1_KEY);
        final Document featuresDoc = (Document) doc.get(FIELD_FEATURES);
        final Document featureDoc = (Document) featuresDoc.get(FEATURE_ID1);
        final Document propertiesDoc = (Document) featureDoc.get(FIELD_PROPERTIES);
    ...
    public void addFeaturesWithSpecialChars() {
        ...
        Assertions.assertThat(((List<Document>) doc.get(FIELD_INTERNAL)).get(0).get("k"))
                .isEqualTo(FIELD_FEATURE_PROPERTIES_PREFIX_WITH_ENDING_SLASH + PROP_KEY_WITH_SPECIAL_CHARS);
        final Document featuresDoc = (Document) doc.get(FIELD_FEATURES);
