...
public final class AggregationPersistenceComplexCriteriaTest extends AbstractVersionedThingSearchPersistenceTestBase {
    ...
    public void findAllWithAndCriteria() {
        final PolicyRestrictedSearchAggregation aggregation1 = abf.newBuilder(
    ...
    public void findAllWithNoredAndCriteria() {
        final PolicyRestrictedSearchAggregation aggregation1 = abf.newBuilder(
                cf.nor(cf.and(Arrays.asList(searchForKey(KNOWN_ATTRIBUTE_KEY_1, KNOWN_STRING_VALUE),
    ...
    public void findAllWithNoredOrCriteria() {
        final PolicyRestrictedSearchAggregation aggregation1 = abf.newBuilder(
                cf.nor(cf.or(Arrays.asList(searchForKey(KNOWN_ATTRIBUTE_KEY_1), searchForKey(KNOWN_ATTRIBUTE_KEY_2)))))
                .authorizationSubjects(KNOWN_SUBJECTS)
                .build();
    ...
    public void findAllWithNoredNorCriteria() {
        final PolicyRestrictedSearchAggregation aggregation1 = abf.newBuilder(
                cf.nor(cf.nor(Arrays.asList(searchForKey(KNOWN_ATTRIBUTE_KEY_1), searchForKey(KNOWN_ATTRIBUTE_KEY_2)))))
                .authorizationSubjects(KNOWN_SUBJECTS)
                .build();
    ...
    private Criteria searchForKey(final String key, final String value) {
        return cf.fieldCriteria(ef.filterByAttribute(key), cf.eq(value));
    }
