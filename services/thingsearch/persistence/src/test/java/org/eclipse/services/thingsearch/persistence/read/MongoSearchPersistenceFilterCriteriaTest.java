 ...
 */
public final class MongoSearchPersistenceFilterCriteriaTest extends AbstractReadPersistenceTestBase {
    ...
    public void neIsEqualToNotEq() {
        final Criteria critNe =
                cf.fieldCriteria(ef.filterByAttribute(KNOWN_STRING_ATTR), cf.ne(THING1_KNOWN_STR_ATTR_VALUE));
        final Collection<String> resultNe = findAll(critNe);

        final Criteria critNotEq = cf.nor(
                Arrays.asList(
                        cf.fieldCriteria(ef.filterByAttribute(KNOWN_STRING_ATTR), cf.eq(THING1_KNOWN_STR_ATTR_VALUE))));
        final Collection<String> resultNotEq = findAll(critNotEq);
    ...
    private void insertDocsForAttributeCriteria() {
        ...
        final Collection<Document> documents = new ArrayList<>(4);
        documents.add(createThingDocument(THING1_ID, attributes1));
        documents.add(createThingDocument(THING2_ID, attributes2));
        documents.add(createThingDocument(THING3_ID, attributes3));
        documents.add(createThingDocument(THING4_ID, attributes4));
