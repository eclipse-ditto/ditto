...
public final class AmqpBridgeRootActor extends AbstractActor {
    ...
    private final SupervisorStrategy strategy = new OneForOneStrategy(true, DeciderBuilder
            .match(NullPointerException.class, e -> {
                ...
                log.error(e,
                        "DittoRuntimeException '{}' should not be escalated to AmqpBridgeRootActor. Simply resuming Actor.",
                        e.getErrorCode());
                return SupervisorStrategy.resume();
    ...
    private void startClusterSingletonActor(final String actorName, final Props props) {
        final ClusterSingletonManagerSettings settings =
                ClusterSingletonManagerSettings.create(getContext().system()).withRole(AMQP_BRIDGE_CLUSTER_ROLE);
