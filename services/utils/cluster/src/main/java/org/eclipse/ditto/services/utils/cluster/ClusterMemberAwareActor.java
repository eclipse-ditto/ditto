...
public final class ClusterMemberAwareActor extends AbstractActor {
    ...
    public Receive createReceive() {
        return ReceiveBuilder.create()
                    ...
                    if (address.host().isDefined()) {
                        try {
                            ...
                            if (knownAddresses.containsKey(inetAddress.getHostName())) {
                                ...
                                log.debug("New WEAKLY UP member '{}' is already known with address '{}'", weaklyUpMember,
                                        knownAddress);

                                final Set<Member> unreachableMembers =
                                        JavaConversions.setAsJavaSet(cluster.state().unreachable());
                ...
                .match(ClusterEvent.UnreachableMember.class, unreachableMemberEvent -> {
                ...
                .match(ClusterEvent.MemberEvent.class,
                        memberEvent -> log.info("Unhandled Member event: {}", memberEvent))
                .match(CheckForMajority.class, checkForMajority -> {
                    majorityCheck = null;
    ...
    private void scheduleMajorityCheck() {
        scheduleMajorityCheck(new CheckForMajority());
    }
