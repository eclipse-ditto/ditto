...
/** A [[akka.event.DiagnosticLoggingAdapter]] with additional functionality.
 * The main purpose of ThreadSafeDittoDiagnosticLoggingAdapter is to provide an API for easily setting a correlation ID
 * to the logging
 * <a href="http://logback.qos.ch/manual/mdc.html">Mapped Diagnostic Context (MDC)</a> <em>before each log
 * operation</em>.
 * <p>
 * Once a usable correlation ID is set to a ThreadSafeDittoDiagnosticLoggingAdapter it will be appended to the log by
 ...
 * <p>
 * <pre>
 * final ThreadSafeDittoDiagnosticLoggingAdapter logger = DittoLoggerFactory.getThreadSafeDiagnosticLoggingAdapter(MyClass.class);
 ...
 * A call of [[ThreadSafeDittoDiagnosticLoggingAdapter discardCorrelationId()]] discards the correlation ID eventually.
 * </p>
 * <p>
 * <pre>
 * ThreadSafeDittoDiagnosticLoggingAdapter logger = DittoLoggerFactory.getThreadSafeDiagnosticLoggingAdapter(MyClass.class); // note that the variable is not final
 ...
 */
@ThreadSafe
abstract class ThreadSafeDittoDiagnosticLoggingAdapter extends DiagnosticLoggingAdapter
  with MdcEntrySettable[ThreadSafeDittoDiagnosticLoggingAdapter] {
   ...
   * [[ThreadSafeDittoDiagnosticLoggingAdapter discardCorrelationId()]].
   *
   * @param correlationId the correlation ID to be put to the MDC before each log operation.
   * @return a ThreadSafeDittoDiagnosticLoggingAdapter which appends the specified correlation ID to all of its log
   *         operations.
   */
  def withCorrelationId(@Nullable correlationId: CharSequence): ThreadSafeDittoDiagnosticLoggingAdapter
   ...
   *                         each log operation.
   * @return a ThreadSafeDittoDiagnosticLoggingAdapter which appends the derived correlation ID to all of its log
   *         operations.
   * @throws NullPointerException if `withDittoHeaders` is `null`.
   * @see #withCorrelationId(DittoHeaders)
   */
  def withCorrelationId(withDittoHeaders: WithDittoHeaders[_]): ThreadSafeDittoDiagnosticLoggingAdapter
   ...
   * effect like [[ThreadSafeDittoDiagnosticLoggingAdapter discardCorrelationId()]].
   *
   * @param dittoHeaders might contain the correlation ID to be put to the MDC before each log operation.
   * @return a ThreadSafeDittoDiagnosticLoggingAdapter which appends the obtained correlation ID to all of its log
   *         operations.
   * @throws NullPointerException if `dittoHeaders` is `null`.
   */
  def withCorrelationId(dittoHeaders: DittoHeaders): ThreadSafeDittoDiagnosticLoggingAdapter
   ...
   *
   * @return a ThreadSafeDittoDiagnosticLoggingAdapter without a correlation ID.
   */
  def discardCorrelationId: ThreadSafeDittoDiagnosticLoggingAdapter
