 ...
 */
public final class DispatcherActorCreatorTng {
    ...
    public static Props props(final ConciergeConfig conciergeConfig, final ActorRef pubSubMediator,
        ...
        return GraphActor.partial(actorContext -> {
            DispatcherActorCreatorTng.initActor(actorContext.self(), pubSubMediator);
            return DispatcherActorCreatorTng.dispatchGraph(actorContext, conciergeConfig, pubSubMediator,
                    enforcerShardRegion);
        });
    }
    ...
    public static Props props(final ConciergeConfig conciergeConfig, final ActorRef pubSubMediator,
        ...
        return GraphActor.partial(actorContext -> {
            ...
            return Flow.<WithSender>create()
                    .via(preEnforcer)
                    .via(DispatcherActorCreatorTng.dispatchGraph(actorContext, conciergeConfig, pubSubMediator,
                            enforcerShardRegion));
        });
    }
    ...
    private static Graph<FlowShape<WithSender, WithSender>, NotUsed> dispatchGraph(
        ...
        return Flow.<WithSender>create()
                .via(DispatcherActorCreatorTng.dispatchSearchCommands(pubSubMediator))
                .via(DispatcherActorCreatorTng.dispatchRetrieveThings(actorContext, conciergeConfig,
                        enforcerShardRegion));
    }
    ...
    private static Graph<FlowShape<WithSender, WithSender>, NotUsed> dispatchSearchCommands(
        ...
        return GraphDSL.create(builder -> {
            ...
            final SinkShape<WithSender<Object>> sinkToSearchActor =
                    builder.add(DispatcherActorCreatorTng.forwardToThingSearchActor(pubSubMediator));
    ...
    private static Graph<FlowShape<WithSender, WithSender>, NotUsed> dispatchRetrieveThings(
            final ActorContext actorContext, final ConciergeConfig conciergeConfig,
            final ActorRef enforcerShardRegion) {

        final Props props = ThingsAggregatorActorTng.props(conciergeConfig, enforcerShardRegion);
    ...
    private static void initActor(final ActorRef self, final ActorRef pubSubMediator) {
        DispatcherActorCreatorTng.sanityCheck(self);
        DispatcherActorCreatorTng.putSelfToPubSubMediator(self, pubSubMediator);
    }
