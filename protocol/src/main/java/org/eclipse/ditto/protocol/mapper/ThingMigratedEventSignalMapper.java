...
/**
 * Signal mapper for {@link ThingMigrated} events.
 */
final class ThingMigratedEventSignalMapper extends AbstractSignalMapper<ThingMigrated> {
    ...
    @Override
    DittoHeaders enhanceHeaders(final ThingMigrated signal) {
        return ProtocolFactory.newHeadersWithJsonMergePatchContentType(signal.getDittoHeaders());
    }

    @Override
    TopicPath getTopicPath(final ThingMigrated signal, final TopicPath.Channel channel) {
