...
```json
{
  "dittoHeadersForMerge": {
    ...
    "put-metadata": [
      {
        "key": "*/updated-by",
        "value": "{%raw%}{{ request:subjectId }}{%endraw%}"
      },
      { "key": "*/updated-via", "value": "device-live-response" },
      {
        "key": "*/update-hint",
        "value": "{%raw%}{{ header:some-custom-hint }}{%endraw%}"
      },
 ...
 */
function buildDittoProtocolMsg(
  namespace,
  name,
  group,
  channel,
...
After instantiation of the custom `MessageMapper`, the `configure` method is called with all the _options_ which were
