...
|---------------|--------------------------------|--------------|----------|
| `fn:filter`   | `(String filterValue, String rqlFunction, String comparedValue)`| Filters the result of the previous expression in case the condition specified by the parameters does not succeed. | `fn:filter(header:response-required,'eq','true')` |
