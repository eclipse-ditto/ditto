...
[Connections](basic-connections.html) additionally accept a placeholder function pipeline (ending with
`fn:filter()`) in a separate `fn-filter` parameter alongside (or instead of) the RQL `filter` -- see
