...
```
http://localhost:8080/api/<1|2>/things?namespaces=org.eclipse.ditto.one,org.eclipse.test&filter=eq(attributes/count,42)
```
...
```
http://localhost:8080/api/<1|2>/things?filter=gt(attributes/foo,42)
```
