...
export async function ready() {
    ...
    dom.showTemplates.onclick = function () {
        Templates.setSelectedService(dom.serviceSelector.value);
    }
...
async function submitPiggybackCommand() {
    if (isCommandValid()) {
        ...
        try {
            let path = `/devops/piggyback${buildPath(
    ...
    function isCommandValid() {
        ...
        let isServiceValid = validateAndReturn(dom.serviceSelector.value !== "", NO_SERVICES_MESSAGE, dom.serviceSelector);
