...
/**
 * This strategy handles {@link SubjectAliasSubjectDeleted} events.
 */
final class SubjectAliasSubjectDeletedStrategy
        extends AbstractPolicyEventStrategy<SubjectAliasSubjectDeleted> {

    @Override
    protected PolicyBuilder applyEvent(final SubjectAliasSubjectDeleted event, final Policy policy,
            final PolicyBuilder policyBuilder) {

        PolicyImports policyImports = policy.getPolicyImports();
        for (final SubjectAliasTarget target : event.getTargets()) {
            ...
            policyImports = currentImports.getPolicyImport(target.getImportedPolicyId())
                    .map(existingImport -> {
                        ...
                        final Subjects existingSubjects =
                                oldAddition.getSubjects().orElse(PoliciesModelFactory.emptySubjects());
                        final Subjects newSubjects = existingSubjects.removeSubject(event.getSubjectId());
                        final EntryAddition newAddition = PoliciesModelFactory.newEntryAddition(
                                target.getEntryLabel(),
                                newSubjects,
                                oldAddition.getResources().orElse(null),
                                oldAddition.getNamespaces().orElse(null));
