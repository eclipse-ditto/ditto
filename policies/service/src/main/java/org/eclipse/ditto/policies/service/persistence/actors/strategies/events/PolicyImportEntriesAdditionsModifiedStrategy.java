...
/**
 * This strategy handles {@link PolicyImportEntriesAdditionsModified} events.
 */
final class PolicyImportEntriesAdditionsModifiedStrategy
        extends AbstractPolicyEventStrategy<PolicyImportEntriesAdditionsModified> {

    @Override
    protected PolicyBuilder applyEvent(final PolicyImportEntriesAdditionsModified event, final Policy policy,
        ...
        return policy.getPolicyImports().getPolicyImport(importedPolicyId)
                .map(existingImport -> {
                    final PolicyImport newImport = reconstructImportWithEntriesAdditions(
                            existingImport, event.getEntriesAdditions());
