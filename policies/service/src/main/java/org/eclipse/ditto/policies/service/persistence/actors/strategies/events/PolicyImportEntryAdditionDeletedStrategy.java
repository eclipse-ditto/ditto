...
/**
 * This strategy handles {@link PolicyImportEntryAdditionDeleted} events.
 */
final class PolicyImportEntryAdditionDeletedStrategy
        extends AbstractPolicyEventStrategy<PolicyImportEntryAdditionDeleted> {

    @Override
    protected PolicyBuilder applyEvent(final PolicyImportEntryAdditionDeleted event, final Policy policy,
        ...
        return policy.getPolicyImports().getPolicyImport(importedPolicyId)
                .map(existingImport -> {
                    ...
                    final EntriesAdditions newAdditions = existingAdditions.removeAddition(event.getLabel());
