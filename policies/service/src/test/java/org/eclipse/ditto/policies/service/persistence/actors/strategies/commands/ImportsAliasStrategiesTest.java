 ...
 */
public final class ImportsAliasStrategiesTest extends AbstractPolicyCommandStrategyTest {
    ...
    public void setUp() {
        ...
        final List<ImportsAliasTarget> targets = Arrays.asList(
                PoliciesModelFactory.newImportsAliasTarget(TEMPLATE_POLICY_ID, TARGET_LABEL_1),
                PoliciesModelFactory.newImportsAliasTarget(TEMPLATE_POLICY_ID, TARGET_LABEL_2)
        );
        final ImportsAlias alias = PoliciesModelFactory.newImportsAlias(ALIAS_LABEL, targets);
        final ImportsAliases aliases = PoliciesModelFactory.newImportsAliases(Collections.singletonList(alias));
    ...
    public void modifySubjectsViaAliasFansOutToAllTargets() {
        ...
        assertModificationResult(modifySubjectsStrategy, policyWithAlias, command,
                ImportsAliasSubjectsModified.class,
                event -> {
    ...
    public void modifySubjectViaAliasCreatesNewSubjectInAllTargets() {
        ...
        // The new subject doesn't exist in the targets yet, so we expect ImportsAliasSubjectCreated
        assertModificationResult(modifySubjectStrategy, policyWithAlias, command,
                ImportsAliasSubjectCreated.class,
                event -> {
                    assertThat(event.getAliasLabel()).isEqualTo(ALIAS_LABEL);
    ...
    public void modifySubjectViaAliasModifiesExistingSubjectInAllTargets() {
        ...
        // Subject already exists → expect ImportsAliasSubjectModified (not Created)
        assertModificationResult(modifySubjectStrategy, policyWithAlias, command,
                ImportsAliasSubjectModified.class,
                event -> {
                    assertThat(event.getAliasLabel()).isEqualTo(ALIAS_LABEL);
    ...
    public void deleteAllImportsAliases() {
        final var deleteImportsAliasesStrategy = new DeleteImportsAliasesStrategy(
                DefaultPolicyConfig.of(ConfigFactory.load("policy-test")));
        final var command = org.eclipse.ditto.policies.model.signals.commands.modify.DeleteImportsAliases.of(
                POLICY_ID, DITTO_HEADERS);

        assertModificationResult(deleteImportsAliasesStrategy, policyWithAlias, command,
                org.eclipse.ditto.policies.model.signals.events.ImportsAliasesDeleted.class,
                event -> {},
                response -> {});
    }
