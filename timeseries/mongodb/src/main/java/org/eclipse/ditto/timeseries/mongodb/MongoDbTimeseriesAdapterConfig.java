...
public interface MongoDbTimeseriesAdapterConfig extends TimeseriesAdapterConfig {
     ...
     * kept until manually purged. An existing collection is reconciled to this value via
     * {@code collMod} the first time its namespace is written to after a restart, so a changed
     * value takes effect without manual migration — but not before that namespace next ingests.
