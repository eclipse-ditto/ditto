...
public class NamespaceAccessValidatorTest {
    ...
    @Test
    public void testGetApplicableNamespacePatternsReturnsExplicitEmptySetForWildcardAllowedNamespaces() {
        final NamespaceAccessConfig config = NamespaceAccessTestSupport.config(
        ...
        assertThat(validator.getApplicableNamespacePatterns())
                .hasValueSatisfying(patterns -> assertThat(patterns).isEmpty());
    }
    ...
    @Test
    public void testMultipleSimultaneouslyMatchingRulesAreConsistent() {
        final NamespaceAccessConfig ruleA = NamespaceAccessTestSupport.config(
                List.of(),
                List.of("ns.a.*"),
                List.of()
        );
        final NamespaceAccessConfig ruleB = NamespaceAccessTestSupport.config(
                List.of(),
                List.of("ns.b.*"),
                List.of("ns.blocked")
        );
        ...
        assertThat(validator.isNamespaceAccessible("ns.a.device")).isTrue();
        assertThat(validator.isNamespaceAccessible("ns.b.device")).isTrue();
        assertThat(validator.isNamespaceAccessible("ns.c.device")).isFalse();
        assertThat(validator.isNamespaceAccessible("ns.blocked")).isFalse();
    }

    @Test
    public void testConditionRetainsNoValuesMeansRuleNotFulfilledAndAccessIsDenied() {
        ...
        // Fail-closed: no rule matched → access denied.
        assertThat(validator.isNamespaceAccessible("com.acme")).isFalse();
    }
    ...
    @Test
    public void testJwtConditionWithoutJwtDeniesAccessFailClosed() {
        ...
        // No JWT present → JWT condition resolves to empty → rule not applied.
        // Fail-closed: no rule matched → access denied for all namespaces.
        assertThat(validator.isNamespaceAccessible("org.eclipse.demo")).isFalse();
        assertThat(validator.isNamespaceAccessible("com.acme")).isFalse();
    }

    @Test
    public void testGetApplicableNamespacePatternsWithJwtConditionWithoutJwtReturnsEmptySet() {
        final NamespaceAccessConfig jwtRule = NamespaceAccessTestSupport.config(
                List.of("{{ jwt:iss | fn:filter('like','http://localhost:9900/eclipse*') }}"),
                List.of("org.eclipse.demo"),
                List.of()
        ...
        // No JWT present → no condition matched → fail-closed: restrict search to nothing.
        assertThat(validator.getApplicableNamespacePatterns())
                .hasValueSatisfying(patterns -> assertThat(patterns).isEmpty());
    }
